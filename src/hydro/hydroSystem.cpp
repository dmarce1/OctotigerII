...
HydroSystem::HydroSystem(Real adiabaticIndex, Real densityFloor, Real pressureFloor)
	: adiabaticIndex_(adiabaticIndex), densityFloor_(densityFloor), pressureFloor_(pressureFloor) {
	if (!(adiabaticIndex_ > 1) || !(densityFloor_ > 0) || !(pressureFloor_ > 0)) {
...
PrimitiveState HydroSystem::reconstructionVariables(ConservedState const& state) const {
	if (!admissible(state)) {
		throw std::runtime_error(
			"Cannot convert an inadmissible hydro state to primitive variables");
	}
...
ConservedState HydroSystem::conservedState(PrimitiveState const& state) const {
	if (!(state.density() >= densityFloor_) || !(state.pressure() >= pressureFloor_) ||
	...
	result.totalEnergy() =
		state.pressure() / (adiabaticIndex_ - 1) + Real(0.5) * state.density() * speedSquared;
	return result;
}
...
ConservedState HydroSystem::riemann(ConservedState const& left, ConservedState const& right,
									int normal) const {
	...
	Real const leftSound =
		std::sqrt(adiabaticIndex_ * leftPrimitive.pressure() / leftPrimitive.density());
	Real const rightSound =
		std::sqrt(adiabaticIndex_ * rightPrimitive.pressure() / rightPrimitive.density());
	...
	Real const leftDenominator =
		leftPrimitive.density() * (leftSpeed - leftPrimitive.velocity(normal));
	Real const rightDenominator =
		rightPrimitive.density() * (rightSpeed - rightPrimitive.velocity(normal));
	Real const denominator = leftDenominator - rightDenominator;
	Real const denominatorScale =
		std::max({Real(1), std::abs(leftDenominator), std::abs(rightDenominator)});
	Real const waveScale = std::max({Real(1), std::abs(leftSpeed), std::abs(rightSpeed)});
	...
	Real const contactSpeed = (rightPrimitive.pressure() - leftPrimitive.pressure() +
							  ...
							  denominator;
	if (!std::isfinite(contactSpeed) || contactSpeed < leftSpeed || contactSpeed > rightSpeed) {
		return hll(left, right, normal);
	}
		...
		star.totalEnergy() =
			...
			(state.totalEnergy() / primitive.density() +
			 (contactSpeed - primitive.velocity(normal)) *
				 (contactSpeed + primitive.pressure() / (primitive.density() * waveDifference)));
		return star;
	};
...
ConservedState HydroSystem::limitFlux(ConservedState const& left, ConservedState const& right,
	...
	Real const speed =
		std::max(maximumSignalSpeed(left, normal), maximumSignalSpeed(right, normal));
	ConservedState const lowOrderFlux =
		Real(0.5) * (leftPhysical + rightPhysical - speed * (right - left));
	if (!validFlux(lowOrderFlux)) {
		throw std::runtime_error(
			"First-order hydro flux is not positivity preserving at this timestep");
	}
	Real low = 0;
	Real high = 1;
...
ConservedState HydroSystem::hll(ConservedState const& left, ConservedState const& right,
								int normal) const {
	PrimitiveState const leftPrimitive = reconstructionVariables(left);
	PrimitiveState const rightPrimitive = reconstructionVariables(right);
	Real const leftSound =
