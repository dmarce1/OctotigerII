...
// Equation numbers below refer to S&O. Output contains physical F;
...
template <typename Type = Real, int dimensionCount = 3> class RadiationM1 {
  ...
  public:
	...
	class ConservedState : public StateVector {
	  public:
		...
		FluxState physicalFlux(int normal, Type chat, Type gridVelocity = 0) const {
			...
			auto const radical = std::sqrt(std::max(
				Type(0), Type(2) * (Type(1) - f2) * (s + mu2 * (s - Type(2))) / (s + Type(1))));
			FluxState out;
			...
			for (int d = 0; d < dimensionCount; ++d) {
				Type const pressure =
					m.directed * fn * m.reducedFlux[d] + (normal == d ? m.isotropic : Type(0));
	...
	static ConservedState roundoffState(ConservedState u, Type updateScale) {
		...
		for (int f = 1; f < fieldCount; ++f)
			magnitude = std::hypot(magnitude, u[f]);
		Type const tolerance = roundoff * updateScale;
		if (u[0] < 0 && -u[0] <= tolerance && magnitude <= tolerance)
			return {};
	...
	static std::pair<ConservedState, ConservedState> reconstruct(ConservedState const& left,
																 ConservedState const& center,
																 ConservedState const& right) {
		FpeGuard guard;
		...
		auto valid = [&](Type a) {
			return admissible(center - a * slope) && admissible(center + a * slope);
		};
		Type a = 1;
		if (!valid(a)) {
			Type lo = 0, hi = 1;
	...
	static ConservedFlux hll(ConservedState const& left, ConservedState const& right, int normal,
							 Type chat, Type gridVelocity = 0) {
		FpeGuard guard;
		auto canonical = [](ConservedState u) {
			u.checkState("HLL input");
			Type magnitude = 0;
			for (int d = 1; d < fieldCount; ++d)
				magnitude = std::hypot(magnitude, u[d]);
			if (magnitude > u[0])
				for (int d = 1; d < fieldCount; ++d)
					u[d] *= u[0] / magnitude;
			return u;
	...
	static ConservedFlux limitFlux(ConservedState const& left, ConservedState const& right,
								   ConservedFlux const& high, int normal, Type chat,
								   Type gridVelocity, Type dtOverDx) {
		if (dtOverDx == 0)
			return high;
		...
		auto validState = [&](StateVector const& u) {
			if (admissible(u))
				return true;
			if (!std::isfinite(u[0]) || u[0] < -tolerance)
				return false;
			Type norm = 0;
			for (int f = 1; f < fieldCount; ++f) {
				if (!std::isfinite(u[f]))
		...
		auto valid = [&](StateVector const& flux) {
			return validState(left - factor * (flux - l)) &&
				   validState(right + factor * (flux - r));
		};
		if (valid(high))
			return high;
		...
		if (!valid(low))
			throw std::runtime_error("M1 first-order flux violates realizability; reduce rad_cfl");
		Type lo = 0, hi = 1;
		for (int it = 0; it < 56; ++it) {
			Type const mid = Type(.5) * (lo + hi);
			if (valid(low + mid * (high - low)))
				lo = mid;
			else
				hi = mid;
		}
