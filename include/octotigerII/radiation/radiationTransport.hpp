...
class RadiationSystem {
  public:
	...
	[[nodiscard]] State limitFlux(State const& left, State const& right, State const& highOrderFlux,
								  int normal, Real stepOverCellWidth, int dimensionCount) const;

	// The transport state is (E,Q=F/c). Output uses physical cgs flux units.
...
template <int dimensionCount> using Solver = physics::MusclHancock<RadiationSystem, dimensionCount>;
