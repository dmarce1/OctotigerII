...
class MeshLayout {
  public:
	...
	PhysicalCoordinates cellCenter(PhysicalCoordinates const& lower, Real cellWidth,
								   Coordinates const& interiorCoordinates) const;

	template <class Function> void forEachInterior(Function&& function) const {
...
template <class State> class PatchData {
  public:
	PatchData() = default;
	PatchData(MeshLayout layout, Real cellWidth, PhysicalCoordinates lower = {})
		: layout_(std::move(layout)), cellWidth_(cellWidth), lower_(lower),
		...
		for (int axis = 0; axis < maximumDimensionCount; ++axis) {
			if (!std::isfinite(lower_[axis]) || (!layout_.isActive(axis) && lower_[axis] != 0)) {
