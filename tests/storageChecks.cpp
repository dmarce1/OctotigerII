...
void fields() {
	...
	for (auto range : layout.ranges()) {
		...
		EXPECT_TRUE(!handle.local(range) || input.data() == output.data()) << "Local checkout copied field data";
		for (std::size_t i = 0; i < range.count; ++i)
			EXPECT_TRUE(input.data()[i] == units::Density::from_value(100 * range.partition + range.offset + i)) << "Field read/write roundtrip";
		...
		for (std::size_t i = 0; i < range.count; ++i)
			EXPECT_TRUE(current.data()[i] == input.data()[i] && future.data()[i] == 2.0 * input.data()[i]) << "Stage banks alias";
		auto slice = handle.read(range.slice(1, range.count - 1), 0).get();
		EXPECT_TRUE(slice.data()[0] == input.data()[1]) << "Subrange read";
	}
	...
	EXPECT_TRUE(other.handle().id != handle.id && other.handle().partitions == handle.partitions) << "Fields did not share a partition with distinct identities";
	for (auto range : layout.ranges()) {
		auto independent = other.handle().read(range, 0).get();
		for (std::size_t i = 0; i < range.count; ++i)
			EXPECT_TRUE(independent.data()[i] == units::Density{}) << "Distinct fields alias";
		auto third = handle.output(range, 2);
		for (std::size_t i = 0; i < range.count; ++i)
			third.data()[i] = units::Density::from_value(42);
		handle.commit(range, 2, third);
		EXPECT_TRUE(handle.read(range, 2).get().data()[0] == units::Density::from_value(42)) << "Configurable bank count failed";
	}
	...
	for (auto range : fluxLayout.ranges()) {
		...
		for (std::size_t i = 0; i < range.count; ++i)
			EXPECT_TRUE(input.data()[i] == units::EnergyFlux::from_value(2.99792458e10 * (i + 1))) << "Physical flux transfer changed units or values";
	}
	bool rejected = false;
	try {
	...
	EXPECT_TRUE(rejected) << "Invalid field address accepted";
	rejected = false;
	try {
	...
	EXPECT_TRUE(rejected) << "Invalid bank accepted";
	...
	EXPECT_TRUE(rejected) << "Mismatched quantity type accepted";
#ifdef OCTOTIGERII_WITH_HPX
	...
	auto checkChunks = [&](auto const& buffer) {
		...
		EXPECT_TRUE(pointerChunk) << "Typed field buffer lost zero-copy serialization";
	};
	...
	flux.retire();
	EXPECT_TRUE(retained.data()[0] == units::EnergyFlux::from_value(2.99792458e10)) << "Retirement invalidated a retained buffer";
	rejected = false;
	try {
	...
	EXPECT_TRUE(rejected) << "Retired field handle accepted";
	storage::Field<units::EnergyFlux> replacement(fluxLayout, store, 1);
	EXPECT_TRUE(replacement.handle().id != flux.handle().id) << "Retired field identity reused";
...
void same(std::vector<Snapshot> const& a, std::vector<Snapshot> const& b) {
	ASSERT_EQ(a.size(), b.size()) << "Snapshot count changed";
	for (std::size_t block = 0; block < a.size(); ++block) {
		EXPECT_TRUE(a[block].time == b[block].time) << "Failed stage changed time";
		EXPECT_TRUE(a[block].layout.ghostWidth() == 0) << "Persistent snapshot contains ghosts";
		for (std::size_t i = 0; i < a[block].radiation.values().size(); ++i)
			a[block].radiation.values()[i].forEach(
				[&](auto field, auto q) { EXPECT_TRUE(q == b[block].radiation.values()[i].template get<field>()) << "Published radiation input changed"; });
		for (std::size_t i = 0; i < a[block].hydro.values().size(); ++i)
			a[block].hydro.values()[i].forEach(
				[&](auto field, auto q) { EXPECT_TRUE(q == b[block].hydro.values()[i].template get<field>()) << "Published input changed"; });
	}
}
...
void stages() {
	...
	EXPECT_TRUE(rejected && runtime.generation() == 0) << "Invalid numerical stage was published";
...
TEST(Storage, IndependentFieldsBanksViewsTypesAndRetirement) {
	fields();
}


#if OCTOTIGERII_HYDRO || OCTOTIGERII_RADIATION
TEST(Runtime, FailedStageRollsBackAndRetryPublishesOnce) {
	stages();
}
#endif
